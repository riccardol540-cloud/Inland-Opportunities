"""Generate the INLAND Rwanda pitch HTML and overview HTML from the deck data."""
import html
...
<title>INLAND Rwanda — Africa–Europe Co-Production</title><style>{CSS}</style></head>
<body>
<div class="hero">
  <div class="mark">INLAND</div>
  <h1>Rwanda</h1><div class="rule"></div>
...
<section style="background:#fff"><div class="wrap"><div class="label">The Research</div><h2 class="sec-h">Rwanda — What Survived</h2><hr>
<p class="lead">{esc(INTRO['research'])}</p><p class="muted">{esc(INTRO['research_scope'])}</p>
<p><span class="stat"><span class="n">20</span><br><span class="label">story leads researched</span></span>
...
<p class="lead" style="margin:14px auto 0;max-width:60ch">Twelve selected stories across communities, ritual, food, ecology, craft and music. Each centres a named custodian, a specific place, and a practice that can still be witnessed.</p></div></section>
...
<div class="tracks">{tracks_html()}</div></div></section>
...
<section class="dark center"><div class="wrap"><h2>12 Storylines. One Rwanda.</h2>
...
<title>INLAND Rwanda — Overview</title><style>{CSS}
...
<h1>Rwanda</h1><div class="sub" style="font-size:1.1rem">An Africa–Europe Cultural Co-Production · Overview</div>
...
open("INLAND_Rwanda_Pitch.html","w").write(PITCH)
open("INLAND_Rwanda_Overview.html","w").write(OVERVIEW)
